...
> Token documentation added 9 Sep 2026 (user audit):
> - `--brand-deep-green`  → alias for `--bg-dark` oklch(16% 0.03 145)
>   Used by .story__badge for the rating pills (apricot pill, deep green
>   stamp text) on the home page story section.
> - `--brand-soft-orange`  → alias for `--accent-soft` oklch(85% 0.06 35)
>   The cream-on-cream tint used elsewhere on the site. Used by
>   .bike-toppings on the ice-cream-bike page to replace the previous
>   dark forest-green band.
